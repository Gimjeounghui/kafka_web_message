...
public class KafkaConsumerConfig {
    ...
    public ConsumerFactory<String, MessageVO> consumerFactory() {
        ...
        return new DefaultKafkaConsumerFactory<>(config, new StringDeserializer(), new JsonDeserializer<>(MessageVO.class));
    }
...
public class KafkaConsumerConfig {
    ...
    private void initializeConsumerFactory() {
        Map<String, Object> config = new HashMap<>();
        config.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, servers);
        config.put(ConsumerConfig.GROUP_ID_CONFIG, "3D-Digital-Twin");
